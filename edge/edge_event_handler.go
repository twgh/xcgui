...
//
// cb: 回调函数. 如果为 nil, 则仅注册事件, 不添加回调函数, 返回回调函数索引为-2.
//
...
func (w *webviewEventBus) AddCallback(impl *WebViewEventImpl, eventType string, cb interface{}, obj interface{}, allowAddingMultiple ...bool) (int, error) {
	...
	newCbInfo := xc.CbInfo{ID: id, CB: cb}
	...
	if isAddingMultiple {
		...
		if cb == nil {
			...
			info.Cbs = []xc.CbInfo{newCbInfo}
		}
	}
	eventMap[eventType] = info
	w.EventInfoMap[impl] = eventMap
	return id, nil
}
...
//
// eventType: 事件类型, 如: NavigationCompleted.
func (w *webviewEventBus) GetHandler(impl *WebViewEventImpl, eventType string) unsafe.Pointer {
	w.mu.RLock()
	defer w.mu.RUnlock()
	return w.EventInfoMap[impl][eventType].EventHandlerPointer
}
...
//
// eventType: 事件类型, 如: NavigationCompleted.
func (w *webviewEventBus) GetEventToken(impl *WebViewEventImpl, eventType string) *EventRegistrationToken {
	w.mu.RLock()
	defer w.mu.RUnlock()
...
//
// eventType: 事件类型, 如: NavigationCompleted.
func (w *webviewEventBus) ReleaseEventHandler(impl *WebViewEventImpl, eventType string) *webviewEventBus {
	w.mu.Lock()
	defer w.mu.Unlock()
	eventMap := w.EventInfoMap[impl]
...
//
// impl: WebViewEventImpl 对象.
func (w *webviewEventBus) ReleaseAllEventHandler(impl *WebViewEventImpl) *webviewEventBus {
	w.mu.Lock()
	defer w.mu.Unlock()
	eventMap := w.EventInfoMap[impl]
...
//
// impl: WebViewEventImpl 对象.
func (w *webviewEventBus) RemoveAllCallback(impl *WebViewEventImpl) *webviewEventBus {
	w.mu.Lock()
	delete(w.EventInfoMap, impl)
	w.mu.Unlock()
	return w
...
//
// eventType: 事件类型, 如: NavigationCompleted.
func (w *webviewEventBus) RemoveCallbacks(impl *WebViewEventImpl, eventType string) *webviewEventBus {
	w.mu.Lock()
	defer w.mu.Unlock()
...
//
// id: 回调函数 ID.
func (w *webviewEventBus) RemoveCallback(impl *WebViewEventImpl, eventType string, id int) *webviewEventBus {
	w.mu.Lock()
	defer w.mu.Unlock()
...
//
// cb: 回调函数.
func (w *webviewEventBus) SetCallback(impl *WebViewEventImpl, eventType string, id int, cb interface{}) *webviewEventBus {
	w.mu.Lock()
	defer w.mu.Unlock()
