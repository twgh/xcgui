...
//   - source：其值设置为 chrome.webview 对象。如果提供的字符串作为 additionalDataAsJson 但不是有效的 JSON 字符串，API 将失败并返回 E_INVALIDARG。如果 access 为 COREWEBVIEW2_SHARED_BUFFER_ACCESS_READ_ONLY，脚本将仅对缓冲区具有读取权限。如果脚本尝试修改只读缓冲区中的内容，将导致 WebView 渲染器进程中出现访问冲突，并使渲染器进程崩溃。如果共享缓冲区已关闭，API 将失败并返回 RO_E_CLOSED。
