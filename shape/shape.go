...
func NewShape(hShape int) *Shape {
	p := &Shape{
		HXCGUI: xc.XShape_GetParentEle(hShape),
	}
	return p
}
...
func (s *Shape) GetHWINDOW() int {
	return xc.XShape_GetHWINDOW(s.HXCGUI)
}

// 形状_取父对象, 获取父对象, 父可能是元素或窗口, 返回父对象
func (s *Shape) GetParent() int {
	return xc.XShape_GetParent(s.HXCGUI)
}

// 形状_移除, 从父UI元素或窗口,和父布局对象中移除
func (s *Shape) RemoveShape() int {
	return xc.XShape_RemoveShape(s.HXCGUI)
}

// 形状_置ID
// nID: ID值.
func (s *Shape) SetID(nID int) int {
	return xc.XShape_SetID(s.HXCGUI, nID)
}

// 形状_取ID
func (s *Shape) GetID() int {
	return xc.XShape_GetID(s.HXCGUI)
}

// 形状_置UID
// nUID: UID值.
func (s *Shape) SetUID(nUID int) int {
	return xc.XShape_SetUID(s.HXCGUI, nUID)
}

// 形状_取UID
func (s *Shape) GetUID() int {
	return xc.XShape_GetUID(s.HXCGUI)
}

// 形状_置名称, 设置形状对象name
// pName: name值
func (s *Shape) SetName(pName string) int {
	return xc.XShape_SetName(s.HXCGUI, pName)
}

// 形状_取名称, 获取形状对象name
func (s *Shape) GetName() string {
	return xc.XShape_GetName(s.HXCGUI)
}

// 形状_取Z序, 获取形状对象Z序, 成功返回索引值, 否则返回 XC_ID_ERROR
func (s *Shape) GetZOrder() int {
	return xc.XShape_GetZOrder(s.HXCGUI)
}

// 形状_重绘, 重绘形状对象
func (s *Shape) Redraw() int {
	return xc.XShape_Redraw(s.HXCGUI)
}

// 形状_取宽度, 获取内容宽度
func (s *Shape) GetWidth() int {
	return xc.XShape_GetWidth(s.HXCGUI)
}

// 形状_取高度, 获取内容高度
func (s *Shape) GetHeight() int {
	return xc.XShape_GetHeight(s.HXCGUI)
}
...
func (s *Shape) Move(x int, y int) int {
	return xc.XShape_Move(s.HXCGUI, x, y)
}

// 形状_取坐标
// pRect: 接收返回坐标.
func (s *Shape) GetRect(pRect *xc.RECT) int {
	return xc.XShape_GetRect(s.HXCGUI, pRect)
}

// 形状_置坐标
// pRect: 坐标.
func (s *Shape) SetRect(pRect *xc.RECT) int {
	return xc.XShape_SetRect(s.HXCGUI, pRect)
}
