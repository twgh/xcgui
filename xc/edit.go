...
// color: RGB颜色值
// alpha: 透明度
func XEdit_SetDefaultTextColor(hEle int, color int, alpha uint8) int {
	r, _, _ := xEdit_SetDefaultTextColor.Call(uintptr(hEle), uintptr(color), uintptr(alpha))
	return int(r)
}
...
// color: RGB颜色
// alpha: 透明度
func XEdit_SetSelectBkColor(hEle int, color int, alpha uint8) int {
	r, _, _ := xEdit_SetSelectBkColor.Call(uintptr(hEle), uintptr(color), uintptr(alpha))
	return int(r)
}
...
// iRow: 行索引
// iCol: 列索引
func XEdit_SetCurPos(hEle int, iRow int, iCol int) int {
	r, _, _ := xEdit_SetCurPos.Call(uintptr(hEle), uintptr(iRow), uintptr(iCol))
	return int(r)
}
...
// iPos: 位置点
// pInfo: 行列
func XEdit_PositionToInfo(hEle int, iPos int, pInfo *Position_) int {
	r, _, _ := xEdit_PositionToInfo.Call(uintptr(hEle), uintptr(iPos), uintptr(unsafe.Pointer(pInfo)))
	return int(r)
}
