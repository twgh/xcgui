...
// color: RGB颜色值
// alpha: 透明度
func XFrameWnd_SetPaneSplitBarColor(hWindow int, color int, alpha uint8) int {
	r, _, _ := xFrameWnd_SetPaneSplitBarColor.Call(uintptr(hWindow), uintptr(color), uintptr(alpha))
	return int(r)
}
