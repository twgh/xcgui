...
// color: RGB颜色值.
// alpha: 透明度.
func XShapeGroupBox_SetBorderColor(hShape int, color int, alpha uint8) int {
	r, _, _ := xShapeGroupBox_SetBorderColor.Call(uintptr(hShape), uintptr(color), uintptr(alpha))
	return int(r)
}
...
// color: RGB颜色值.
// alpha: 透明度.
func XShapeGroupBox_SetTextColor(hShape int, color int, alpha uint8) int {
	r, _, _ := xShapeGroupBox_SetTextColor.Call(uintptr(hShape), uintptr(color), uintptr(alpha))
	return int(r)
}
