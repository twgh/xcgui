...
// alpha: 透明度.
// width: 线宽.
func XListBox_AddItemBkBorder(hEle int, nState int, color int, alpha uint8, width int) int {
	r, _, _ := xListBox_AddItemBkBorder.Call(uintptr(hEle), uintptr(nState), uintptr(color), uintptr(alpha), uintptr(width))
	return int(r)
}
...
// color: RGB颜色.
// alpha: 透明度.
func XListBox_AddItemBkFill(hEle int, nState int, color int, alpha uint8) int {
	r, _, _ := xListBox_AddItemBkFill.Call(uintptr(hEle), uintptr(nState), uintptr(color), uintptr(alpha))
	return int(r)
}
...
// nState: 项状态.
// hImage: 图片句柄.
func XListBox_AddItemBkImage(hEle int, nState int, hImage int) int {
	r, _, _ := xListBox_AddItemBkImage.Call(uintptr(hEle), uintptr(nState), uintptr(hImage))
	return int(r)
}

// 列表框_取项背景对象数量
// hEle: 元素句柄.
func XListBox_GetItemBkInfoCount(hEle int) int {
	r, _, _ := xListBox_GetItemBkInfoCount.Call(uintptr(hEle))
	return int(r)
}

// 列表框_清空项背景对象
// hEle: 元素句柄.
func XListBox_ClearItemBkInfo(hEle int) int {
	r, _, _ := xListBox_ClearItemBkInfo.Call(uintptr(hEle))
	return int(r)
}
...
// pArray: 数组缓冲区.
// nArraySize: 数组大小.
func XListBox_GetSelectAll(hEle int, pArray int, nArraySize int) int {
	r, _, _ := xListBox_GetSelectAll.Call(uintptr(hEle), uintptr(pArray), uintptr(nArraySize))
	return int(r)
}
...
func XListBox_SetItemFloat(hEle int, iItem int, iColumn int, fFloat float32) bool {
	r, _, _ := xListBox_SetItemFloat.Call(uintptr(hEle), uintptr(iItem), uintptr(iColumn), uintptr(fFloat))
	return int(r) != 0
}
...
func XListBox_SetItemFloatEx(hEle int, iItem int, pName string, fFloat float32) bool {
	r, _, _ := xListBox_SetItemFloatEx.Call(uintptr(hEle), uintptr(iItem), strPtr(pName), uintptr(fFloat))
	return int(r) != 0
}
