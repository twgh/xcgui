...
func XFont_Create2(pName string, size int, style int) int {
	r, _, _ := xFont_Create2.Call(strPtr(pName), uintptr(size), uintptr(style))
	return int(r)
}

// 字体_创建3, 创建炫彩字体, 返回字体句柄.
// pInfo: 字体信息.
func XFont_Create3(pInfo *Font_Info_) int {
	r, _, _ := xFont_Create3.Call(uintptr(unsafe.Pointer(pInfo)))
	return int(r)
}

// 字体_创建扩展, 创建炫彩字体, 返回字体句柄.
// pFontInfo: 字体信息.
func XFont_CreateEx(pFontInfo *LOGFONTW) int {
	r, _, _ := xFont_CreateEx.Call(uintptr(unsafe.Pointer(pFontInfo)))
	return int(r)
}
