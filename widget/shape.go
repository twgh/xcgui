...
func (s *Shape) GetHWINDOW() int {
	return xc.XShape_GetHWINDOW(s.Handle)
}

// 形状_取父对象, 获取父对象, 父可能是元素或窗口, 返回父对象
func (s *Shape) GetParent() int {
	return xc.XShape_GetParent(s.Handle)
}
...
func (s *Shape) SetID(nID int) int {
	return xc.XShape_SetID(s.Handle, nID)
}

// 形状_取ID
func (s *Shape) GetID() int {
	return xc.XShape_GetID(s.Handle)
}

// 形状_置UID
// nUID: UID值.
func (s *Shape) SetUID(nUID int) int {
	return xc.XShape_SetUID(s.Handle, nUID)
}

// 形状_取UID
func (s *Shape) GetUID() int {
	return xc.XShape_GetUID(s.Handle)
}

// 形状_置名称, 设置形状对象name
// pName: name值
func (s *Shape) SetName(pName string) int {
	return xc.XShape_SetName(s.Handle, pName)
}

// 形状_取名称, 获取形状对象name
func (s *Shape) GetName() string {
	return xc.XShape_GetName(s.Handle)
}
