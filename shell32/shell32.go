...
func DragQueryFileW(hDrop, iFile int, lpszFile *string, cch int) int {
	buf := make([]uint16, cch)
	r, _, _ := dragQueryFileW.Call(uintptr(hDrop), uintptr(iFile), xc.Uint16SliceDataPtr(&buf), uintptr(cch))
...
func ShellExecuteW(hwnd int, lpOperation, lpFile, lpParameters, lpDirectory string, nShowCmd xcc.SW_) int {
	r, _, _ := shellExecuteW.Call(uintptr(hwnd), xc.StrPtr(lpOperation), xc.StrPtr(lpFile), xc.StrPtr(lpParameters), xc.StrPtr(lpDirectory), uintptr(nShowCmd))
	return int(r)
}
