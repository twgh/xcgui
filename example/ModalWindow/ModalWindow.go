...
func main() {
	...
	// 创建按钮_模态窗口
	btn_Modal := widget.NewButton(30, 50, 75, 30, "ModalWindow", win.Handle)
	// 给按钮绑定事件
	btn_Modal.Event_BnClick(func(pbHandled *bool) int {
		// 创建模态窗口
		win_Modal := window.NewModalWindow(300, 200, "模态窗口", win.GetHWND(), xcc.Xc_Window_Style_Modal|xcc.Xc_Window_Style_Drag_Window)
		// 设置模态窗口边框
		win_Modal.SetBorderSize(1, 25, 1, 1)
		// 在模态窗口上创建关闭按钮
		widget.NewButton(50, 50, 70, 30, "close", win_Modal.Handle).SetType(xcc.Button_Type_Close)
		// 显示模态窗口
		win_Modal.DoModal()
		return 0
	})
