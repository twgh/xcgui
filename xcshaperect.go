...
// color: RGB颜色值.
// alpha: 透明度.
func XShapeRect_SetBorderColor(hShape int, color int, alpha uint8) int {
	r, _, _ := xShapeRect_SetBorderColor.Call(uintptr(hShape), uintptr(color), uintptr(alpha))
	return int(r)
}
...
// color: RGB颜色值.
// alpha: 透明度.
func XShapeRect_SetFillColor(hShape int, color int, alpha uint8) int {
	r, _, _ := xShapeRect_SetFillColor.Call(uintptr(hShape), uintptr(color), uintptr(alpha))
	return int(r)
}
