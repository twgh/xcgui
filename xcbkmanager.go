...
// alpha: 透明度.
// width: 线宽.
func XBkM_AddBorder(hBkInfoM int, nState int, color int, alpha uint8, width int) int {
	r, _, _ := xBkM_AddBorder.Call(uintptr(hBkInfoM), uintptr(nState), uintptr(color), uintptr(alpha), uintptr(width))
	return int(r)
}
...
// color: RGB颜色.
// alpha: 透明度.
func XBkM_AddFill(hBkInfoM int, nState int, color int, alpha uint8) int {
	r, _, _ := xBkM_AddFill.Call(uintptr(hBkInfoM), uintptr(nState), uintptr(color), uintptr(alpha))
	return int(r)
}
